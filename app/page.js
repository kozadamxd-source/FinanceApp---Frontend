...
export default function Home() {
  ...
  const loadData = async () => {
    ...
    try {
      const [mRes, hRes] = await Promise.all([
        fetch(`${API}/metrics?symbol=${symbol}`),
        fetch(`${API}/history?symbol=${symbol}&days=30`),
      ]);
      ...
      chartInstance.current = new Chart(chartRef.current, {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (ctx) => `${ctx.parsed.y.toFixed(2)} PLN`,
              },
            },
          },
          scales: {
  ...
  const priceChange = history
    ? (
        ((history.history[history.history.length - 1].close -
          history.history[0].close) /
          history.history[0].close) *
        100
      ).toFixed(2)
    : null;
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0f0e1a", color: "#fff", fontFamily: "sans-serif" }}>
      {/* Sidebar */}
      <div style={{ width: 180, background: "#13121f", padding: "24px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
        <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 28, display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ color: "#a78bfa", fontSize: 20 }}>📈</span> FinanceApp
        </div>
        ...
        <div style={{ marginTop: "auto", padding: "9px 12px", borderRadius: 10, fontSize: 13, color: "#888", cursor: "pointer" }}>
          Ustawienia
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: 24, overflowY: "auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              placeholder="np. CDPROJEKT, KGHM..."
              value={symbol}
              onChange={(e) => setSymbol(e.target.value.toUpperCase())}
              onKeyDown={(e) => e.key === "Enter" && loadData()}
              style={{ background: "#1e1c30", border: "0.5px solid #2d2b45", borderRadius: 8, padding: "7px 12px", color: "#fff", fontSize: 13, width: 220, outline: "none" }}
            />
            <button
              onClick={loadData}
